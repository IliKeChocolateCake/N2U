...
class RequestTopUp extends StatefulWidget {
  final String amount;
  ...
  @override
  State<RequestTopUp> createState() => _RequestTopUpState();
}

class _RequestTopUpState extends State<RequestTopUp> {
  ...
  Future<void> _submitTopUpRequest() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(requestTopUp),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
        body: jsonEncode({
          'top_up_amount': double.parse(widget.amount),
        }),
      );

      final data = jsonDecode(response.body);
      debugPrint('📦 Top-up Response: $data');
      ...
      if (response.statusCode == 200 && data['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(data['message'] ?? 'Failed to submit top-up request'),
            backgroundColor: Colors.red,
          ),
        );
      }
    } catch (e) {
      debugPrint('Error submitting top-up: $e');
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(preferredSize: Size.fromHeight(kToolbarHeight), child: Container(
        ...
        child:    AppBar(
          backgroundColor: Colors.white,
          centerTitle: true,
          title:   Text('Confirm Top Up', style: GoogleFonts.dmSans(fontSize: 18,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text(
                        '• Your top-up request will be pending until approved',
                        style: GoogleFonts.dmSans(
                          fontSize: 14,
                      ...
                      Text(
                        '• Funds will be added to your Dine-in Wallet',
                        style: GoogleFonts.dmSans(
                          fontSize: 14,
                          color: Colors.grey.shade700,
                        ),
